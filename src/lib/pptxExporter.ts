...
function buildSlideXml(layout: SlideLayout, content: Record<string, string>): string {
  const shapes = layout.placeholders
    .map((p, i) => {
      const idAttr = i + 2; // 1 is reserved for the group
...
function rewriteSldIdLst(doc: Document, relId: string): void {
  ...
  // Insert after sldMasterIdLst if present, else at top of presentation
...
export async function exportActiveSlideAsPptx(
  ...
  // Clone bytes so a re-export doesn't see a previously mutated zip view.
  ...
  // 1. Slide XML
  const slideXml = buildSlideXml(layout, slide.content);
  zip.file("ppt/slides/slide1.xml", slideXml);

  // 2. Slide rels → layout
  zip.file("ppt/slides/_rels/slide1.xml.rels", buildSlideRelsXml(layoutFileName));
