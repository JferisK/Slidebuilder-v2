...
3. Otherwise, open `src/stores/slideStore.ts`, inspect the store shape, and ask the user to share the current `theme.cssVars`.
...
Produce the `# Template Context` block from the canonical spec. Hand it to the next role as-is.
