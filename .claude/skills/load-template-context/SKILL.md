...
3. Otherwise, `Read` `src/stores/slideStore.ts` to understand the store shape, and ask the user to share the `theme.cssVars` for the currently loaded PPTX.
...
Return the `# Template Context` block from the canonical spec. Hand it to the next role as-is.
